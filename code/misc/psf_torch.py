...
class PsfGenerator3D:
    # Adopted from https://github.com/maweigert

    def __init__(self, psf_shape, units, lam_detection, n, na_exc, n_threads=4):
        ...
        self.kcut = 1. * na_exc / self.lam_detection
    ...
    # Each Zernike polynomial defined individually below. Following ANSI standard.
    ...
    def masked_displaced_phase_array(self, phi, normalized=True, piston_tip_tilt=False, pupil_shape='circle', 
                                     Dx=None, 
                                     Dy=None, 
                                     Dz=None, 
                                     theta=None, 
                                     ...
                                     My=None,
                                     mask=True):
        # Dx, Dy in the Fourier domain
        # Dz on the image plane
        # theta in degrees.
        ...
        if mask:
            ...
            kmask2 = torch.ones_like(_phase)

        
        if Dz is None and Dy is None and Dx is None and theta is None and Mx is None and My is None:
            return torch.fft.fftshift(kmask2 * _phase)
            
        else:
            if Dx is not None:
                _phase = torch.fft.fftshift(_phase)
                _phase = torch.from_numpy(scipy.ndimage.shift(_phase.detach().cpu().numpy(), 
                                                              shift=(0, Dx))).type(dtype).cuda(0)
                _phase = torch.fft.fftshift(_phase)

            if Dy is not None:
                _phase = torch.fft.fftshift(_phase)
                _phase = torch.from_numpy(scipy.ndimage.shift(_phase.detach().cpu().numpy(), 
                                                              shift=(Dy, 0))).type(dtype).cuda(0)
                _phase = torch.fft.fftshift(_phase)

            if theta is not None:
                _phase = torch.fft.fftshift(_phase)
                _phase = torch.from_numpy(scipy.ndimage.rotate(_phase.detach().cpu().numpy(),
                                                               angle=theta, 
                                                               reshape=False)).type(dtype).cuda(0)
                _phase = torch.fft.fftshift(_phase)

            if Mx is not None and My is not None:
                assert Mx >= 1 and My >= 1
                _phase = torch.fft.fftshift(_phase)
                _phase = torch.from_numpy(scipy.ndimage.zoom(_phase.detach().cpu().numpy(), (My, Mx))).type(dtype).cuda(0)
                _phase = _phase[_phase.shape[0]//2-self.Ny//2:_phase.shape[0]//2+self.Ny//2,
                                _phase.shape[1]//2-self.Nx//2:_phase.shape[1]//2+self.Nx//2]
                _phase = torch.fft.fftshift(_phase)
                                                     
            if Dz is not None:
                Hdisp = torch.sqrt(1. * self.n ** 2 - self.KR2 ** 2 * self.lam_detection ** 2).type(dtype).cuda(0)       
                out_ind = torch.isnan(Hdisp)
                kprop = torch.exp(-2.j * np.pi * Dz / self.lam_detection * Hdisp)
                kprop[out_ind] = 0.
                ku = kmask2 * kprop * torch.exp(2.j * torch.pi * _phase / self.lam_detection)

                return torch.fft.fftshift(kmask2 * torch.angle(ku) * self.lam_detection / 2 / torch.pi)

            else:
                return torch.fft.fftshift(kmask2 * _phase)
                
    
    def masked_phase_array(self, phi, normalized=True, piston_tip_tilt=False):
        _phase = torch.zeros_like(self.krho)
    ...
    def coherent_psf(self, phi, normalized=True, piston_tip_tilt=False):
    ...
    def incoherent_psf(self, phi, normalized=True, piston_tip_tilt=False):
